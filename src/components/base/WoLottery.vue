  ...
  methods: {
    ...
    startLottery() {
      ...
      // if(this.currentEnv =='notSmallRoutine'){
        this.$get(`/atpapi/act/lottery/start/${version}/actPath/${actCode}/${this.type}`).then((res) => {
        if (res.code === '0000') {
          if (res.data) {
            ...
            if (res.data.prizeType === 1) {
              ...
              this.currentModal.content.img = this.config.lottery.prizes[this.targetCell - 1]
              this.currentModal.content.smallText = res.data.prizeDesc
              this.currentModal.content.url = res.data.url
            } else if (res.data.prizeType === 4) {
              // 未中奖
              this.currentModal = this.config.modal.noPrize
            }else if(res.data.prizeType === 5){
              this.currentModal = this.config.modal.noPrize
              this.currentModal.content.midText = ""
              this.currentModal.title = "恭喜中奖"
              this.currentModal.btn.text = "再来一次"
              this.currentModal.content.noimg = "https://atp.bol.wo.cn/atp_resource/upload//atp_resource/2020/09/22/a2e78254953a82f7c937137aff29ab6d.png"
            }else if(res.data.prizeType === 6){
              this.currentModal = this.config.modal.virtualPrize
              this.currentModal.content.img = this.config.lottery.prizes[this.targetCell - 1]
              this.currentModal.content.smallText = res.data.prizeDesc
              this.currentModal.content.url = res.data.url
