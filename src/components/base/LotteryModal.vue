        ...
        <div
          ...
          <div class="recordarea" v-if="config.name === 'myPrize'">
            <div
              ...
              >
              <div class="record">
                <div class="prizename" v-if="record.type == 1  || record.type == 3 || record.type == 5 || record.type == 6">{{record.name}}</div>
                ...
                <div
                  class="operation"
                  v-if="record.type == 2 || record.type == 6 && record.url"
...
export default {
  ...
  computed:{
    btnshow() {
      let btnisShow = false
      if (this.config.name === 'codePrize' || this.config.name === 'virtualPrize') {
        btnisShow = this.config.content.url? true: false
      }else if (this.config.btn.isShow) {
        btnisShow = this.config.btn.isShow
      }
      return btnisShow
    }
  },
  methods: {
    ...
    clickEvent() {
      ...
      const url = this.config.content.url
